...
const applicationsStatics = (arr) => {
    // write your code here
    return arr.reduce((result, cv) => {
        result.totalApplicants++;
        if (!cv.firstName && !cv.lastName || cv.yearsOfExperience <= 1) {
...
const classesAvg = (data) => {
    ...
    for (let i = 0; i < data.grades.length; i++) {
        ...
        for (let j = 0; j < grade.classes.length; j++) {
            ...
            let sum = 0;
            for (let k = 0; k < classData.classScores.length; k++) {
                sum += classData.classScores[k];
            }
            let avg = Math.floor(sum / classData.classScores.length);
            classData.avg = avg;
        }
    }
    return data;
